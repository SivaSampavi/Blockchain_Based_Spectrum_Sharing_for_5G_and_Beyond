...
Implementation of a registrar based on second-price blind advertisements and funds held on deposit, with a renewal process that weights renewal costs according to the change in mean price of registering a domain. Largely untested!
...
alternatively, deploy the `HashRegistrar`:
    ...
    $ truffle migrate --network dev.advertisement
